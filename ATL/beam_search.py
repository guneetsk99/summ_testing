...
class Hypothesis(object):
  ...
  def __init__(self, tokens, log_probs, state, attn_dists, p_gens, coverage, coverage_pool):
  ...
  def extend(self, token, log_prob, state, attn_dist, p_gen, coverage, coverage_pool):
...
def run_beam_search(sess, model, vocab, batch):
  ...
  hyps = [Hypothesis(tokens=[vocab.word2id(data.START_DECODING)],
                     ...
                     coverage_pool=np.zeros([batch.patch_padding_mask.shape[1]])
  ...
  while steps < FLAGS.max_dec_steps and len(results) < FLAGS.beam_size:
    ...
    prev_coverage_pool = [h.coverage_pool for h in hyps]

    # Run one step of the decoder to get the new info
    (topk_ids, topk_log_probs, new_states, attn_dists, p_gens, new_coverage, new_coverage_pool) = model.decode_onestep(sess=sess,
                                                                                                                       batch=batch,
                                                                                                                       latest_tokens=latest_tokens,
                                                                                                                       enc_states=enc_states,
                                                                                                                       dec_init_states=states,
                                                                                                                       prev_coverage=prev_coverage,
                                                                                                                       prev_coverage_pool=prev_coverage_pool)
    ...
    for i in xrange(num_orig_hyps):
      h, new_state, attn_dist, p_gen, new_coverage_i, new_coverage_pool_i = hyps[i], new_states[i], attn_dists[i], p_gens[i], new_coverage[i], new_coverage_pool[i]  # take the ith hypothesis and new decoder state info
      for j in xrange(FLAGS.beam_size * 2):  # for each of the top 2*beam_size hyps:
        ...
        new_hyp = h.extend(token=topk_ids[i, j],
                           ...
                           coverage_pool=new_coverage_pool_i)
