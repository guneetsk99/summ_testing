...
def example_generator(data_path, img_feature_path, single_pass, max_img_num=7):
  ...
  while True:
    ...
    text_img_pairs = zip(filelist, feature_list)

    if not single_pass:
      random.shuffle(text_img_pairs)
    ...
    for text_f, img_f in text_img_pairs:
      ...
      while True:
        ...
        img_feature = img_feature.reshape((-1, 512))
        img_idx += 1
        str_len = struct.unpack('q', len_bytes)[0]
        example_str = struct.unpack('%ds' % str_len, reader.read(str_len))[0]
        yield (example_pb2.Example.FromString(example_str), img_feature)
    if single_pass:
